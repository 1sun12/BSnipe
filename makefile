...
#
# -MMD    = While compiling, also write a "obj/main.d" file listing every header
#           that "src/main.c" included. Make reads those back in (see -include
#           below) so editing a .h rebuilds every .c that uses it
#
# -MP     = Adds a dummy rule for each header, so deleting or renaming a header
#           doesn't break the build with "No rule to make target"
#
# -include = Same as "include" but stays quiet when the files don't exist yet
#            (they won't on the very first build, before anything is compiled)
...
# ~ Dependency files (one per object), generated by -MMD while compiling
DEPS = $(OBJECTS:.o=.d)
...
# ~ FLAGS is needed here too, otherwise a sanitizer build compiles but fails to link
$(TARGET): $(OBJECTS) | $(BIN_DIR)
	@$(CC) $(FLAGS) $(OBJECTS) -o $@

# ~ Compile source files to objects (and record which headers each one used)
$(OBJ_DIR)/%.o: $(SRC_DIR)/%.c | $(OBJ_DIR)
	@$(CC) $(FLAGS) -MMD -MP -I$(INC_DIR) -c $< -o $@

# ~ Pull in those header lists so touching a .h rebuilds whatever included it
-include $(DEPS)
